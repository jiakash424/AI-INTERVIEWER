...
def test_Chatbot(openai_create):
    at = AppTest.from_file("Ai_interviewer.py").run()
    assert not at.exception
    at.chat_input[0].set_value("Do you know any Code?").run()
    ...
    at.chat_input[0].set_value("Do you know any Ai assistant?").run()
    print(at)
    assert at.chat_message[1].markdown[0].value == "Do you know any Ai assistant?"
...
def test_Langchain_Quickstart(langchain_llm):
    at = AppTest.from_file("pages/2_Help_Section.py").run()
    assert at.info[0].value == "Please add your OpenAI API key to continue."
